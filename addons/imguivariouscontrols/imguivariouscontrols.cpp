...
bool InputComboWithAutoCompletion(const char* label, int *current_item, size_t autocompletion_buffer_size, InputComboWithAutoCompletionData* pAutocompletion_data,
    ...
    if (!pad->inputTextShown) {
        ...
        float comboWidth = (g.NextItemData.Flags&ImGuiNextItemDataFlags_HasWidth) ? g.NextItemData.Width : window->ItemWidthDefault;
               // (window->DC.ItemWidthStack.size()>0 ? window->DC.ItemWidthStack[window->DC.ItemWidthStack.size()-1] : window->ItemWidthDefault);
        if (comboWidth>ImGui::GetContentRegionAvail().x) comboWidth = ImGui::GetContentRegionAvail().x;
        bool noButtons = false;
        if (comboWidth>buttonsWidth) comboWidth-=buttonsWidth;
        else noButtons=true;
        if (comboWidth<=0.f || window->SkipItems)   {
            ImGui::PopID();
            return false;
        }
...
bool BeginTimeline(const char* str_id, float max_value, int num_visible_rows, int opt_exact_num_rows, ImVec2 *popt_offset_and_scale)
{
    ...
    if (opt_exact_num_rows>0) {
        ...
        ImGui::Private::CalcListClipping(s_timeline_num_rows, row_height, &s_timeline_display_start, &s_timeline_display_end);
...
void EndTimeline(int num_vertical_grid_lines,float current_time,ImU32 timeline_running_color)    {
    ...
    if (current_time<=0)			win->DrawList->AddRectFilled(start, end, color, rounding);
        ...
        if (median.x<startx) median.x=startx;
            ...
            win->DrawList->AddRectFilled(start, median, timeline_running_color, rounding,/*1|8*/ImDrawFlags_RoundCornersLeft);
        }
        median.y=start.y;
        if (median.x<startx+columnWidth) {
            win->DrawList->AddRectFilled(median, end, color, rounding,/*2|4*/ImDrawFlags_RoundCornersRight);
...
bool PasswordDrawer(char *password, int passwordSize,ImGuiPasswordDrawerFlags flags,const float size,const ImU32 colors[7])   {
    ...
    int num_segments = 0;   // <=0 => automatic
...
unsigned int CheckboxFlags(const char* label,int* flags,int numFlags,int numRows,int numColumns,unsigned int flagAnnotations,int* itemHoveredOut,const unsigned int* pFlagsValues)  {
    IM_ASSERT(flags);
...
bool ImGui::ColoredButtonV1(const char* label, const ImVec2& size_arg, ImU32 text_color, ImU32 bg_color_1, ImU32 bg_color_2,float frame_rounding_override)    {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
